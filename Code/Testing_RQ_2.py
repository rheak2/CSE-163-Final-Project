...
This file tests all of the functions implemented for the second research
question, which determines the effectiveness of conservation efforts
for each species in the dataset (that also has corresponding location
data in the API dataset).
'''
import pandas as pd
from utils import process_conservation_data, csv_processing
from utils import species_threat_level_data_processing, process_big_data
...
How have conservation efforts affected the extinction and endangerment of
mammals?
"""
...
def main():
    ...
    # Convert the integers of the conservation rating to floats so that map
    # legend is a gradient
    conservation_data["Num Index"] = conservation_data["Num Index"
                                                       ].astype(float)
    # Merge geometry data and conservation data, and then add geometry column
    # to conservation data DataFrame
    merged_data = conservation_data.merge(gdf, left_on="Country",
    ...
    # Create a colour coded map showing the average extinction threat level
    # change 2007-2021 around the world
    ...
    print(threat_data)
    location_threat_geometry = threat_data.merge(gdf, left_on="Location",
                                                 right_on="name", how="inner")
    print(location_threat_geometry)
    location_threat_geometry = gpd.GeoDataFrame(location_threat_geometry,
                                                geometry="geometry")
    # Plot a map of the average extinction threat level change around the world
    fig, ax = plt.subplots(nrows=1, figsize=(15, 7))
    gdf.plot(color="#EEEEEE", ax=ax)
    location_threat_geometry.plot(column="Average TL Change Over Time",
                                  ax=ax, legend=True)
    ...
    scatter_plot_df["Average Yearly Threat Level Change 2007-2021"
                    ] = threat_data["Average TL Change Over Time"]
    scatter_plot_df["Conservation Efforts 2021"
                    ] = conservation_data["Num Index"]
    scatter_plot_df = scatter_plot_df.dropna()
    sns.relplot(data=scatter_plot_df, x="Conservation Efforts 2021",
                y="Average Yearly Threat Level Change 2007-2021")
